//display none or block function
const display = (id,blockOrNone) => {
  document.getElementById(id).style.display = blockOrNone
}
//empty value
const emptyValue = (name,salary) => {
...
const slider = () => {
  // slider added
  setTimeout(()=> {
    display("modal-make","block")
  })
  setTimeout(()=> {
    document.getElementById("modals").style.transform = "translateY(-50%) translateX(-50%)"
  },300)
}
//save new item
document.addEventListener("DOMContentLoaded", () => {
  ...
  btnShowLast.addEventListener("click", function showLastHandler(e) {
    display("dialog-error","none")
    display("dialog-last-item","block")
    display("dialog-record-count","none")
    slider()
    showLastItem();
  });
  ...
  document
    ...
    .addEventListener("click", function recordCountHandler(e) {
      display("dialog-error",'none')
      display("dialog-record-count",'block')
      display("dialog-last-item",'none')
      slider()
...
function addRecordHandler() {
  ...
  if (!name || !salary || isNaN(salary)) {
    display("dialog-error",'block')
    showDataError(name, salary);
    emptyValue("name","salary")
    return;
  }
...
function toggleErrorMessage(selector, value, msg) {
  if (value) {
    display(selector + "Line","none")
  } else {
    display(selector + "Line","block")
